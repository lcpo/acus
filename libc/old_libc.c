...
//!---------------------------------------------------------------------
///Len type
uni libc_lenx(long long num){
uni i = 0;
if(num==0){i++;}
if(num<0){num= -num; i++;} while(num){num/=16;++i;}return i;
}
//--------------------------------------------------------------------------------
...
//!---------------------------------------------------------------------
char* libc_itox(unsigned long int num){ ///int or long to hex str num
    int i=0,temp,sign=0,j=0;
     uni l=libc_lenx(num);
     char* shex=malloc(l+1);
	 shex+=0;
...
i = 0;
j=l-1;
while(j>i) {swap(shex[i],shex[j]);i++; j--;}  
  shex[l]='\0';
return shex;  
	}
//!---------------------------------------------------------------------
...
//!---------------------------------------------------------------------
uni libc_fprint_str(uni file,char * buf){ //Печатает строку в файл
return write(file, (void*)buf, libc_strlen(buf));
}
//!---------------------------------------------------------------------
...
//!------------------------------------------------------------
size libc_file_chmod(char* filename){
stat_f *fi=malloc(1);
stat(filename,fi);
int out=fi->oct_mode;
free(fi);
...
//!------------------------------------------------------------
int libc_is_dir(char* filename){
stat_f *fi=malloc(1);
stat(filename,fi);
int out=S_ISDIR(fi->st_mode);
free(fi);
return out;
	}							
//!------------------------------------------------------------
int libc_is_file(char* filename){
stat_f *fi=malloc(1);
stat(filename,fi);
int out=S_ISREG(fi->st_mode);
free(fi);
return out;
	}							
...
fstat(res,fi);
char* buff=malloc(fi->st_size);
read(res,buff,fi->st_size);
close(res);
free((void*)fi);
return buff;
										}
...
void parsed_url_free(parsed_url* purl){
            free_ptr(purl->scheme);
            free_ptr(purl->host);
            free_ptr(purl->port);
            free_ptr(purl->path);
            free_ptr(purl->query);
            free_ptr(purl->fragment);
            free_ptr(purl->username);
            free_ptr(purl->password);
            free_ptr(purl->tmpstr);
            free_ptr(purl->curstr);
			free_ptr(purl->ourl);
            free_ptr(purl);
}
///---------------------------------------------------------------------
parsed_url *parse_url(char *url){
    parsed_url *purl=malloc(1);
    purl->tmpstr=malloc(libc_strlen(url));
    purl->curstr=malloc(libc_strlen(url));
    purl->scheme = malloc(libc_strlen(url));
    purl->username = malloc(libc_strlen(url));    
    purl->password = malloc(libc_strlen(url));
    purl->host = malloc(libc_strlen(url));
    purl->port = malloc(libc_strlen(url));
    purl->path = malloc(libc_strlen(url));
    purl->query = malloc(libc_strlen(url));
    purl->fragment = malloc(libc_strlen(url));
	purl->ourl=malloc(libc_strlen(url));
    int len;
    int i;
    int userpass_flag;
    int bracket_flag;

    libc_strcpy(purl->curstr,url);
    libc_strcpy(purl->ourl,libc_strchr(url, ':'));
    libc_strcpy(purl->tmpstr,purl->ourl);


    len = libc_strlen(purl->curstr)-libc_strlen(purl->tmpstr);

    for ( i = 0; i < len; i++ ) {
        if ( !_is_scheme_char(purl->curstr[i]) ) { break;}//??break
    }
    echo("len:",libc_itos(len),"\n");
    libc_strncpy(purl->scheme, purl->curstr, len);
    purl->scheme[len] = '\0';
    ...
    purl->tmpstr++;
    libc_strcpy(purl->curstr,purl->tmpstr);

    for ( i = 0; i < 2; i++ ) {
        if ( '/' != *purl->curstr ) { } //??break;
        purl->curstr++;
    }


    userpass_flag = 0;
    libc_strcpy(purl->tmpstr,purl->curstr);
    while ( '\0' != *purl->tmpstr ) {
        if ( '@' == *purl->tmpstr ) {
            userpass_flag = 1;
            break;
    ...
    if ( userpass_flag ) {
        ...
        while ( '\0' != *purl->tmpstr && ':' != *purl->tmpstr && '@' != *purl->tmpstr ) {
            purl->tmpstr++;
        }
        //len = tmpstr - curstr;
        len = libc_strlen(purl->curstr)-libc_strlen(purl->tmpstr);

        (void)libc_strncpy(purl->username, purl->curstr, len);
        purl->username[len] = '\0';
        ...
        if ( ':' == *purl->curstr ) {
            ...
            while ( '\0' != *purl->tmpstr && '@' != *purl->tmpstr ) {purl->tmpstr++;}
             len = libc_strlen(purl->curstr)-libc_strlen(purl->tmpstr);
            (void)strncpy(purl->password, purl->curstr, len);
            purl->password[len] = '\0';
            libc_strcpy(purl->curstr,purl->tmpstr);
    ...
    libc_strcpy(purl->tmpstr,purl->curstr);
    while ( '\0' != *purl->tmpstr ) {
        if ( bracket_flag && ']' == *purl->tmpstr ) {

            purl->tmpstr++;
            break;
        } else if ( !bracket_flag && (':' == *purl->tmpstr || '/' == *purl->tmpstr) ) {
    ...
    libc_strncpy(purl->host, purl->curstr, len);
    purl->host[len] = '\0';
    libc_strcpy(purl->curstr,purl->tmpstr);
    ...
    if ( ':' == *purl->curstr ) {
        ...
        libc_strcpy(purl->tmpstr,purl->curstr);
        while ( '\0' != *purl->tmpstr && '/' != *purl->tmpstr ) {
            purl->tmpstr++;
        }
    //    len = tmpstr - curstr;
    len = libc_strlen(purl->curstr)-libc_strlen(purl->tmpstr);
        libc_strncpy(purl->port, purl->curstr, len);
        purl->port[len] = '\0';
        libc_strcpy(purl->curstr,purl->tmpstr);
    }
    ...
    //len = tmpstr - curstr;
    len = libc_strlen(purl->curstr)-libc_strlen(purl->tmpstr);
    libc_strncpy(purl->path, purl->curstr, len);
    purl->path[len] = '\0';
    libc_strcpy(purl->curstr,purl->tmpstr);
    ...
    if ( '?' == *purl->curstr ) {
        ...
        len = libc_strlen(purl->curstr)-libc_strlen(purl->tmpstr);
        libc_strncpy(purl->query, purl->curstr, len);
        purl->query[len] = '\0';
        libc_strcpy(purl->curstr,purl->tmpstr);
    }
    ...
    if ( '#' == *purl->curstr ) {
        ...
        //len = tmpstr - curstr;
        len = libc_strlen(purl->curstr)-libc_strlen(purl->tmpstr);
        libc_strncpy(purl->fragment, purl->curstr, len);
        purl->fragment[len] = '\0';
        libc_strcpy(purl->curstr,purl->tmpstr);
    }
...
///------------------------------------------------------------
char* libc_get_file_extension(char* file){
int n=0;
while(libc_sch_count(file++,'.')>0){n++;}
n--;
file--;	
char* out=malloc(libc_strlen(file));
libc_strcpy(out,file);	
file=file-n;
return out;	
	}
///------------------------------------------------------------
char* libc_filename(char* file){
int n=0;
while(libc_sch_count(file++,'/')>0){n++;}
n--;
file--;	
...
///------------------------------------------------------------
size_t libc_strlento(char* str, char ch){
	int n=0;
	if (!str){return n;}
	for (; *str!=ch; str++){n++;}
	return n-1;	
